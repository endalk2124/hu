        ...
        if (!$email) {
            ...
            try {
                ...
                $stmt = $pdo->prepare("
                    ...
                    VALUES (?, ?, ?, ?, 'pending')
                ");
                $stmt->execute([$email, password_hash($temp_password, PASSWORD_DEFAULT), $email, $role]);
                ...
                if ($role === 'student') {
                    $stmt = $pdo->prepare("
                        INSERT INTO students (user_id, email, first_name, last_name) 
                        VALUES (?, ?, ?, ?)
                    ");
                } elseif ($role === 'instructor') {
                    $stmt = $pdo->prepare("
                        INSERT INTO instructors (user_id, email, first_name, last_name) 
                        VALUES (?, ?, ?, ?)
                    ");
                } elseif ($role === 'admin') {
                    $stmt = $pdo->prepare("
                ...
                $stmt->execute([$user_id, $email, $first_name, $last_name]);
...
// Fetch all users from the database with first_name and last_name
try {
    $stmt = $pdo->query("
        SELECT 
            u.user_id,
...
$filtered_users = array_filter($users, function ($user) use ($search, $tab) {
    $matches_search = empty($search) ||
        stripos(($user['student_first_name'] ?? '') . ' ' . ($user['student_last_name'] ?? ''), $search) !== false ||
        stripos(($user['instructor_first_name'] ?? '') . ' ' . ($user['instructor_last_name'] ?? ''), $search) !== false ||
        stripos(($user['admin_first_name'] ?? '') . ' ' . ($user['admin_last_name'] ?? ''), $search) !== false ||
                            ...
                            <tbody>
                                ...
                                <?php else: ?>
                                    <?php foreach ($filtered_users as $user): ?>
                                        <tr>
                                            <td><?= htmlspecialchars($user['student_first_name'] ?? $user['instructor_first_name'] ?? $user['admin_first_name'] ?? '') ?></td>
                                            <td><?= htmlspecialchars($user['student_last_name'] ?? $user['instructor_last_name'] ?? $user['admin_last_name'] ?? '') ?></td>
